...
class Login(QWidget):
    ...
    def pasteLabel(self):
        ...
        label_info.resize(80,20)
        label_info.move(100,240)
        label_info.setFont(QFont('Arial', 8))
        label_info.setText("是否为管理员:")
        label_info.setAutoFillBackground(True)

        self.radioBox=QRadioButton(self)
        self.radioBox.resize(20,20)
        self.radioBox.move(250,240)
    ...
    def loginothers(self):
        ...
        isManager=self.radioBox.isChecked()
        username=self.edit_userName.text()
        password=self.edit_password.text()
        if isManager==False:
            if serviceobj.LoginUser(username,password):
                self.hide()
                self.s = mainWidget.firstWeidget()
                self.s.show()
            else:
                reply = QMessageBox.information(self, "提示", "用户名或密码错误", QMessageBox.Yes | QMessageBox.No ,  QMessageBox.Yes )
        else:
